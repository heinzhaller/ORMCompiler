 ...
 * @version 0.1 beta
 */
abstract class GameInBase extends BaseObject { 
	...
	public function getGame(){
		if( !$this->_getIsLoaded('ref_game') )
			$this->setGame(GameInManager::getGameByGameIn($this));
	 ...
	 */
	public function setGameinid($integer){
		if(is_null($integer))
			throw new ExceptionObject(ExceptionKeys::ERROR_VALUE_NULL, array('Attribute: gameinid'));
		if(is_integer($integer)){
			if( $this->gameinid !== $integer ){
				$this->gameinid = $integer;
				$this->_setModified('gameinid');
			}
		}else{
			throw new ExceptionObject(ExceptionKeys::ERROR_VALUE_WRONGDATATYPE,array('Attribute: gameinid | Type: integer',$integer));
		}
	}
	 ...
	 * @return integer
	 */
	public function getGameinid(){
		return $this->gameinid;
	}
